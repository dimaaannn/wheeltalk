...
public sealed class LabSettings
{
    ...
    /// не задано. Стенду она нужна, потому что на шкалу «по числу ячеек» пускается только слово
    /// человека, а записи такого слова не несут (план 27 §27.4).
    /// </summary>
    public int CellsInSeries { get; set; }

    /// <summary>
    /// Подделать банки умного BMS: записи стенда их не несут, а шкала «по BMS» без них молча
    /// вернётся к вольтам пакета — то есть проверить её было бы нечем. Считается делением пакета на
    /// ряд с небольшим перекосом, чтобы два режима на глаз различались.
    /// </summary>
    public bool FakeBms { get; set; }
