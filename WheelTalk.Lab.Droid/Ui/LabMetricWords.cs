...
public static class LabMetricWords
{
    private static readonly Dictionary<string, string> Words = new(StringComparer.Ordinal)
    {
        ...
        // подписи — «км/ч», «ШИМ %», «V», «t°», «Заряд %» (решение владельца 12.08.2026). Ключ у
        // знака тот же, что у полного имени, плюс «Sign», и берётся он раньше короткого имени.
        ["TelemetrySpeedSign"] = "км/ч",
        ["TelemetryPwmSign"] = "ШИМ %",
        ["TelemetryVoltageSign"] = "V",
        ["TelemetryBoardTempSign"] = "t°",
        ["TelemetryBatterySign"] = "Заряд %",

        // Имена, годные стоять сами по себе, — для меню правки центра, где места вдоволь (решение
        // владельца 12.08.2026: «сокращения только для панели»). Заведены лишь тем величинам, чьё
        // имя в ресурсах названо от раздела экрана «Данные» и в одиночку не читается.
        ["TelemetryBoardTempFull"] = "Температура",
        ["TelemetryTripFull"] = "Пробег",
