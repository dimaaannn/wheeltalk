...
public class TileTypographyTests
{
    ...
    private static TileMetrics Grid() => new(
        // Клетка режется так же, как её режет укладчик: ширина списка без его полей (6 dp с каждой
        // стороны), делённая на двенадцать. Просветы в клетку не входят — плитка врезается внутрь
        // неё, и вычитает их сама ширина плитки.
        CellWidthPx: (ScreenPx - (6 * 2)) / 12,
        RowHeightPx: 68,
        GapPx: 3,
        PaddingPx: 8,
        LabelHeightPx: 16,
    ...
    /// </para>
    /// <para>
    /// <b>11.08.2026 широкие пересняты снова: 64 → 61.</b> Бюджет ширины стал считать плитку так,
    /// как её кладёт укладчик (просветы вычитаются, клетка режется по ширине списка), а единицу —
    /// так, как её рисует плитка: пробелом-знаком и тем же моноширинным начертанием, только мельче.
    /// Оба промаха жили за счёт запаса, который съел якорь запятой, и вышли наружу пустыми плитками
    /// на стенде. Класс 6×1 снова не сдвинулся — его держит высота.
    /// </para>
    /// <para>
    /// <b>И ещё раз в тот же день: 61 → 57.</b> Рамка жара идёт по всем четырём сторонам, а бюджет
    /// вычитал её только сверху; с якорем запятой число доехало до боковой линии рамки, и её край
    /// срезал последний знак на телефоне. Теперь рамка вычитается и по ширине — тем же числом, что
    /// и по высоте.
    /// </para>
    /// </summary>
    [Theory]
