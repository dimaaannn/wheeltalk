...
**Живой объект настроек — один на процесс.** Читатели и писатели держат тот самый экземпляр, что
приезжает из `IOptions<T>.Value`; `IOptionsMonitor`/`IOptionsSnapshot` в приложении запрещены — у
них свой кэш и свой экземпляр, правок в первом они не видят, компилятору второй экземпляр не
виден, а стреляет он через неделю. 09.08.2026 через `IOptionsMonitor<WheelOptions>.CurrentValue` в
историю плиток приезжал адрес колеса, с которым приложение запустилось: после смены колеса графики
плиток продолжали показывать прежнее. Замок от рецидива — `SingleLiveOptionsTests` (план 29 §29.2).
...
- `Settings/` — три слоя (см. выше) и механика описаний. `LayeredSettings` — единственная
  ...
  <br>**`Apply` и `AfterEdit` — разные события, и путать их дорого.** `Apply` кладёт значение в
  живой объект и зовётся на восстановлении: старт приложения, смена слоя, смена колеса, правка
  соседней строки. `AfterEdit` зовётся **только** из `SettingsBinder.Set`, то есть когда значение
  изменил человек, — туда и вешают последствие. Действие, повешенное на `Apply`, срабатывает,
  когда человек ничего не делал: пароль InMotion так поднимал `WheelSession` на старте приложения,
  раньше обработчиков падений (разбор 08.08.2026, [план 21](android-plan-21-kingsong-inmotion.md)).
...
| Кадровый цикл основного экрана | `MainScreenDriver` (библиотека, план 19 Б2) | хозяин считает `MainScreenFrame`, экран его показывает (контракт `IMainScreen`, план 23 шаг 1) |
| Моргание тревоги, наклон панели | сама панель в `Show(MainScreenFrame)` | — свойства панели, не хозяина |
...
| **KingSong** | `KingsongAdapter.java` (713) | Фикстуры (15 тестов) + **живой KS-16S 03.08.2026: разговор начинается** ([план 21](android-plan-21-kingsong-inmotion.md), шапка) — телеметрию на ходу ещё не смотрели |
...
- **Активные протоколы активны по-разному.** Gotway опрашивает «V»/«N» таймером; KingSong говорит
  первым (запрос имени по таймеру на подключении), а дальше спрашивает недостающее на **каждом
  уведомлении** — у оригинала обе половины вне адаптера, в `MainActivity` и `BluetoothService`;
  InMotion шлёт пароль шесть раз и держит поллинг 25 мс. Всё это живёт внутри декодеров через
  `WriteRequested`.
- **«Реагирует на кадр» и «реагирует на уведомление» — разные вещи.** У KingSong запрос стоит вне
  `decode()`, поэтому проверки длины и заголовка его не гасят. Порт сложил их вместе, и живой
  KS-16S 03.08.2026 остался немым: его девятибайтные `AT+ULKTE` до проверки длины не доживали.
  Переносишь активный протокол — смотри **где именно** в оригинале стоит запрос, а не только что
  он шлёт.
