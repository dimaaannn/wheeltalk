...
| **22** | **`transportMode` (toggle)** | **Питание/удержание** | **`fallProtectionAngle`** | b6: transportMode=**2**, оба остальных=**0** в `Ld`-кадре (в `Lk`-половине пары у обоих b6=`0x80`); питание vs fallProtection различаются только формой хвоста — совпадают шестнадцать байт из восемнадцати: у питания последние два байта тела всегда `01 80`, у fallProtection последний байт — переменное значение угла (35..75), предпоследний — заполнитель `0x80` |
...
#### 7.3.1 Опкод 22 — четыре ответа для сборки очереди C (проверено 16.08.2026)

**1. Список литералов полон — других нет.** Искал не по знакомым экранам, а сплошным разбором:
взяты **все** фигурные литералы во всём декомпилате пакета `com.laoniao.leaperkim`, начинающиеся
с `76` (`'L'`), — и `new byte[]{…}`, и объявления вида `byte[] X = {…}`. Всего таких литералов
кадров **45**; с опкодом 22 — ровно **пять**, те самые (§1.4 плана импорта). Шестого нет.

> **Ловушка, из-за которой поиск по числу «22» ненадёжен.** Один литерал прячет опкод за именем
> чужой константы: `CMD_CLEAR_METER = {76, 107, 65, 112, FileDownloadStatus.toFileDownloadService, 0, 1}`
> (`BtManager.java:76`), где `toFileDownloadService = 11` (`FileDownloadStatus.java:17`) — то есть
> опкод 11, записанный не числом. Текстовый `grep` по «22» такой кадр бы пропустил. В разборе всех
> 45 литералов **этот случай единственный**: у прочих байт опкода — число. Если замок когда-нибудь
> будут перепроверять на новой сборке, проверять надо разбором литералов, а не поиском по числу.

**2. Байт 16 у угла защиты — константа, не вычисляется.** В обоих кадрах пары стоит литеральный
`Byte.MIN_VALUE` (`0x80`), а значение угла пишется **только в последний байт**:
`(byte) progressToValue(i)`, где `progressToValue(i) = i + 35` (`SetFallProtectionAngleActivity.java:17,64`).
Ни одной ветки, где предпоследний байт зависел бы от значения, нет. Значит различение опасной пары
по байту 16 (`01` у питания против `80` у угла) надёжно **по построению источника**, а не по
совпадению чисел.

**3. Проверки диапазона перед отправкой нет вовсе.** Границу задаёт только ползунок:
`android:max="40"` (`layout_set_safe_angle.xml:45`) плюс смещение `+35` — отсюда и берутся 35..75.
В коде между ползунком и отправкой **ни одного `if`**, обрезания или отказа. Отправка происходит
в `onStopTrackingTouch` — при каждом отпускании ползунка, без кнопки подтверждения.
**Для нас:** повторять нечего — у производителя границы нет, есть только вид ползунка. Проверку
диапазона придётся ставить свою, и она будет строже оригинала. Это осознанное расхождение, а не
недосмотр порта.

**4. `b6 = 2` у транспортного режима — не совпадение, а общая форма записи.** Пара `(b5=1, b6=2)`
стоит у **шестнадцати** одиночных `Ld`-кадров записи настроек — опкоды 15, 17, 18, 20, 21, 22, 23,
24, 25, 26, 28, 29, 31, 33, 34. Пара `(b5=1, b6=0)` — у **одиннадцати** кадров, и все они вторые
половины парных посылок (опкоды 12, 13, 16, 17, 20, 22). Транспортный режим попадает в первую
группу и **собирается общим сборщиком без особой ветки**.

> **Оговорка, снимающая ложную уверенность:** признаком записи следует считать **пару `(b5, b6)`,
> а не один `b6`**. Есть три `Ld`-кадра вне обеих групп, и один из них ломает правило «`b6=2` значит
> запись»: `CMD_CLEAR_METER_NEW = {76,100,65,112, 13, `**`0, 2`**`, 80, 1}` (`BtManager.java:85`) —
> сброс счётчика поездки, где `b6=2`, но `b5=0`. Остальные два — `CMD_OLDCMDB_NEW` (опкод 14,
> `BtManager.java:82`) и синхронизация времени (опкод 18, `Util.java:236`, `b5/b6 = 0/5`).

**Побочно — питание/удержание, для описания, не для сборки.** Команда запускает **10-секундный
отсчёт до выключения**; повторная посылка сбрасывает отсчёт на новые 10 с. Проверено на живом
колесе владельцем. Отдельной команды отмены нет — отсчёт прерывается повтором той же команды либо
физическим движением колеса. Приложение не даёт нажать её на ходу или на зарядке: показывает
**«Cant Set while riding or charging»** (`set_close_in_10_hint`, `HomepageFragment.java:192-196`).

**Чего в источнике нет:** проверки диапазона угла (см. п. 3); отдельной команды отмены отсчёта
выключения; шестого литерала опкода 22.
...
### 7.7 Поколения педалей — какой модели какая команда

Источник: сборка `com.laoniao.leaperkim` (`versionCode="59"`, `versionName="1.4.8"`,
`C:\Work\repos\loeuc\src_leaper\resources\AndroidManifest.xml`), уже частично разобрана в
[`../../loeuc/leaperkim-official-app.md`](../../loeuc/leaperkim-official-app.md), §5 («Модели и
поколения») — часть этого раздела опирается на неё, часть (все места вызова
`getCarInfoByHardVersion` и что каждое из них шлёт колесу при записи) добавлена здесь заново.
**Пометки достоверности:** «прочитано» — со ссылкой `файл:строка`; «слово владельца, 16.08.2026» —
полевое знание, не из декомпилята.

#### 7.7.1 Встроенный каталог — полная таблица

Единственный источник каталога — константа `Util.CAR_DATA_JSON` (`Util.java:27`, одна строка,
`"version": 9`), разбираемая `CarDataManager.parseResponse` (`CarDataManager.java:62-96`). Десять
записей, из них восемь — реальные модели, две — служебные заглушки для прошивальщика:

| `sNum` | Модель | `hardwareVersion` | `SysVol` | `continuousSoftHardSet` | Заметка |
|---|---|---|---|---|---|
| 10 | Patton-S | `0070` | 126V | **true** | |
| 9 | Sherman-L | `0060` | 151.2V | **true** | |
| 8 | LYNX | `0050` | 151.2V | **true** | |
| 7 | Patton | `0040` | 126V | false | |
| 6 | Sherman-s | `0030` | 100.8V | false | |
| 5 | ShermanMax | `0011` | 100.8V | false | |
| 4 | Abrams | `0020` | 100.8V | false | |
| 3 | Sherman | `0010` | 100.8V | false | |
| 2 | «Firmware downloader» | `6661` | 4.2V | true | заглушка донгла-программатора, не колесо |
| 1 | «Firmware downloader» | `6660` | 4.2V | true | то же, второй профиль |

(все десять — `Util.java:27`, прочитано целиком построчным разбором JSON-константы).

**Oryx, Lynx S, весь Nosfet в каталоге сборки 59 нет вовсе** — их `hardwareVersion` (по шагу `+0x10`
в таблице выше это были бы `0080`/`0090`, но эти строки в константе не встречаются) в перечне
отсутствует. Искал: весь `sources/**/*.java` (только 4 файла ссылаются на `continuousSoftHardSet`,
все разобраны ниже), `resources/res/values/*.xml` (там только массив `ride_mode`, §7.7.5, не
каталог моделей), `resources/assets/**` (только `aax.bin` и профили AOT-компиляции, ни одного
JSON), отдельного файла каталога нет. `resources/classes.dex` декомпилируется в те же `sources/**`,
второго источника внутри APK нет. **Значение `continuousSoftHardSet` для этих моделей в разобранной
сборке не встречается — закрыто словом владельца (16.08.2026): на всех новых моделях LeaperKim
шкала плавная.** Независимого подтверждения из декомпилята нет.

#### 7.7.2 `CarBaseInfo` — полный состав карточки

Восемь полей, все читаются в `CarDataManager.parseResponse` (`CarDataManager.java:74-83`) из
одноимённых (кроме одного) ключей JSON:

| Поле бина (`CarBaseInfo.java`) | Ключ JSON | Тип |
|---|---|---|
| `name` | `name` | строка |
| `sNum` | `sNum` | число |
| `gifUrl` | `gifUrl` | строка (URL картинки) |
| `continuousSoftHardSet` | `continuousSoftHardSet` | bool |
| `SysVol` | `SysVol` | строка |
| `hardwareVersion` | `hardwareVersion` | строка |
| `imgUrl` | `imgUrl` | строка (URL картинки) |
| `volToBattery` | `volToSoc` | список чисел (кривая напряжение→заряд) |

`CarBaseInfoResponse` — обёртка ответа: `version` (число, см. §7.7.3) и `data` (список
`CarBaseInfo`) (`CarBaseInfoResponse.java:7-8`).

#### 7.7.3 Откуда карточка приезжает — не с сервера

`CarDataManager.initCarData()` (`CarDataManager.java:34-37`, зовётся один раз из
`MainActivity.java:182`) сперва поднимает **закэшированный** каталог из локального хранилища
(`AppDataManager.KEY.KEY_CAR_INFOS_DATA`), затем зовёт `requestCarInfo()`
(`CarDataManager.java:51-60`) — и вот это **не сетевой запрос**: метод повторно разбирает ту же
встроенную константу `Util.CAR_DATA_JSON` и, только если её `version` больше версии уже
закэшированного каталога, перезаписывает и кэш, и `mCarBaseInfoResponse`. Имя `requestCarInfo`
вводит в заблуждение — никакого HTTP-вызова внутри нет. Искал отдельный API-слой для каталога:
`grep` по `http://`/`https://` в `sources/com/laoniao/leaperkim/utils/*.java` находит только
`Util.java` (сама константа с URL картинок), выделенного Retrofit/API-класса под каталог машин в
проекте нет (`find *Api*.java *Retrofit*.java *Service*.java` не даёт ничего своего для car-data).
`gifUrl`/`imgUrl` в каталоге указывают на статический CDN
(`https://prodf9b0809-pic4.ysjianzhan.cn/upload/...`) — это только картинки, не данные каталога.

Практический смысл версионирования: `version` в константе растёт от сборки к сборке приложения;
после обновления приложения на телефоне пользователя старый кэш (записанный прошлой версией
приложения) при следующем запуске сравнивается с новой встроенной константой и перетирается ей.
Живого обновления каталога без обновления самого приложения не предусмотрено.

#### 7.7.4 `getCarInfoByHardVersion` — все места вызова и ветка `null`

`CarDataManager.getCarInfoByHardVersion(str)` (`CarDataManager.java:39-49`) — линейный поиск по
каталогу; при пустом/незагруженном каталоге либо при отсутствии совпадения возвращает `null`, без
исключений. Четыре места вызова (плюс сама функция), от единственного, что реально пишет в колесо,
до чисто отображающих:

| Вызов | Пишет в колесо? | Ветка `carInfoByHardVersion == null` | Ветка найдено |
|---|---|---|---|
| `SetRideModeActivity.java:69` | **да** | см. ниже, отдельно | см. ниже, отдельно |
| `ControlActivity.java:374` (`initBaseData`) | нет, только подпись строки меню | текст = число `getRideMode()-100` (`:376`) | `continuousSoftHardSet=true` → то же число (`:379`); `=false` → индекс в `R.array.ride_mode` (`:383`) |
| `HomepageFragment.java:336` (`initCarType`) | нет, только название модели на главном экране | **ничего не делает** — `tvCarType` не трогается вовсе (`:336-339`, нет `else`-ветки) | `tvCarType.setText(carInfoByHardVersion.getName())` (`:338`) |
| `CarInfoActivity.java:75` (`initCarType`) | нет, только карточка «О колесе» | `tvCarType.setText("UNKNOWN")` (литерал, не строковый ресурс) + картинка-заглушка `R.drawable.img_patton2` (`:76-78`) | имя и `imgUrl` из карточки (`:80-81`) |
| `Util.java:85` (`volToBattery`) | нет, расчёт процента заряда | обобщённая формула по напряжению `getBatteryFromLongBitVol(i)` (`:86-87`) | таблица `volToBattery` конкретной модели (`:89`) |

**`SetRideModeActivity` — единственный путь записи, разбор по веткам** (`:66-90`). Весь блок
обёрнут в два вложенных условия: `BtManager.getInstance().getCarDataPackageInfo() != null` (`:66`)
и внутри — `CarDataManager.getInstance().getmCarBaseInfoResponse() != null` (`:68`). Если хоть одно
ложно — **не выполняется ни одна из веток ниже**, в частности не навешивается слушатель на
`radioGroup` (он вешается только внутри `else`, `:81-86`).

- `carInfoByHardVersion == null || carInfoByHardVersion.isContinuousSoftHardSet()` (`:70`) →
  показывается ползунок `layoutOneTo100` (`:71-73`). Запись — `onStopTrackingTouch` →
  `sendCmd(seekBar.getProgress())` (`:62-64`) → пара кадров опкода 12 со значением
  `progressToSendValue(progress) = progress + 10` (`:23-25,118-120`):
  `{76,107,65,112,12,1,0x80,(значение)}` (`Lk`) + `{76,100,65,112,12,1,0,(значение)}` (`Ld`).
  Прогресс бегунка — 0..100 (`android:max="100"`,
  `resources/res/layout/layout_set_ride_mode.xml:96`), значит в кадр уходит число 10..110.
- Иначе (карточка найдена **и** `continuousSoftHardSet == false`) → показывается `radioGroup` из
  трёх кнопок (`:74-80`), слушатель вешается тут же (`:81-86`). Запись — по выбранной кнопке
  уходит текстовая команда `"SETs"`/`"SETm"`/`"SETh"` (`:97-105`), которую `BtManager.sendData`
  превращает в пару кадров опкода 12 со значением 1/2/3 (`BtManager.java:241-246`, байты —
  `CMD_SETS`/`CMD_SETM`/`CMD_SETH` + `_NEW`-варианты, `BtManager.java:77-79,86-88`):
  `{76,107,65,112,12,1,0x80,1|2|3}` (`Lk`) + `{76,100,65,112,12,1,0,1|2|3}` (`Ld`).
- **Каталог не загружен вовсе** (`getmCarBaseInfoResponse() == null`, например до первого прохода
  `initCarData`) — обе ветки выше не выполняются, интерфейс остаётся на разметке по умолчанию:
  ни `layout_oneTo3` (`layout_set_ride_mode.xml:30-54`), ни `layout_oneTo100` (`:55-100`) не имеют
  атрибута `android:visibility` — по умолчанию Android **оба видимы одновременно**. При этом
  слушатель на бегунок навешан безусловно, до всех проверок (`SetRideModeActivity.java:51-65`,
  строки идут раньше блока `:66-90`) — значит **бегунок работоспособен и шлёт кадр опкода 12 со
  значением 10..110 даже без каталога вовсе**, а кнопки трёх положений в этом состоянии инертны
  (слушатель на них не навешан). Это прочитано из кода, не выведено: при полном отсутствии знания о
  модели приложение молча ведёт себя так, будто колесо поддерживает плавную шкалу — тот же исход,
  что и при явном `null` от `getCarInfoByHardVersion`, но по другой причине (отсутствие слушателя, а
  не явная проверка).

**Уточнение к §7.3.** Опкод 12 — не всегда одна команда со значением 1/2/3, как там записано:
для моделей с `continuousSoftHardSet == true` (или отсутствующих в каталоге) тот же опкод несёт
значение 10..110 по формуле выше. Коллизии как таковой нет (диспетчеризация по значению байта не
нужна — колесо либо понимает 1/2/3, либо понимает диапазон, в зависимости от прошивки), но диапазон
значений этого опкода шире, чем там сказано.

#### 7.7.5 «Strong» и «Hard» — три экрана, а не два

Утверждение в `android-plan-34-wheel-settings.md:222` («Hard» на экране настроек против «Strong» на
приборной панели) неточно тем, что мест на самом деле три, и оригинал сам с собой расходится:

| Экран оригинала | Подпись | Источник |
|---|---|---|
| Список настроек (`ControlActivity`, строка меню «Ride Mode setting») | **«Strong»** (индекс 2 массива `R.array.ride_mode`) | `ControlActivity.java:383`, массив `resources/res/values/arrays.xml:3-7` |
| Главный экран/приборная панель (`HomepageFragment`, живой индикатор режима) | **«Strong»** (тот же массив, без проверки `continuousSoftHardSet` — обёрнуто только в `try/catch`, проглатывающий выход за границы массива) | `HomepageFragment.java:80,323-327` |
| Экран записи (`SetRideModeActivity`, третья кнопка `tv_hard`) | **«Hard»** (`R.string.mode_hard`) | `resources/res/layout/layout_set_ride_mode.xml:48-53`, `resources/res/values/strings.xml:177` |

Тот же экран записи использует `mode_hard_short` («Hard» без сокращений в самом тексте) как подпись
конца шкалы 0..100 в режиме ползунка (`layout_set_ride_mode.xml:79`, `strings.xml:178`) — то есть
«Hard» встречается на экране записи **дважды**, в обеих его раскладках.

Итог: «Strong» — это список настроек **и** приборная панель (два места, не одно), «Hard» — только
экран записи режима. Дихотомия «настройки против панели» из плана 34 не разводит первые два экрана,
хотя оба они — часть «настроек» в широком смысле, и оба говорят «Strong»; расходится с ними только
экран, где значение реально записывается.

**Чего в источнике нет.** Значения `continuousSoftHardSet` для Oryx/Lynx S/Nosfet — не найдено ни в
одном месте сборки 59 (см. §7.7.1); закрыто словом владельца, не декомпилятом. Обновления каталога
моделей с сервера производителя — не найдено (§7.7.3): весь механизм локальный. Формула перевода
исходного положения бегунка (`valueToProgress`, `getRideMode()-90`, `SetRideModeActivity.java:32-34,73`)
не разбиралась — вне рамок этого раздела, отмечена только формула записи.
