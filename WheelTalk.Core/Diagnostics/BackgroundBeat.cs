...
/// который работа возобновилась: пришёл кадр с колеса, вернулся экран или поднялся новый процесс).
/// Показ человеку бывает часами позже обнаружения — телефон лежит с погашенным экраном, — и без
/// момента обнаружения сообщение не привязать к журналу.
/// </summary>
public readonly record struct BackgroundGap(TimeSpan Missed, DateTimeOffset NoticedAt);

/// <summary>
/// Отметка «работа шла»: когда в последний раз доказано работали и за каким делом. Пишется на диск
/// редко (<see cref="Period"/>) и только пока работа не закончена, читается на следующем
/// возобновлении работы — из файла, если процесса больше нет, или из памяти, если процесс жив, но
/// его морозили.
/// <para>
/// Момент здесь именно <b>работы</b>, а не последнего вздоха таймера, и это разные вещи ровно в том
/// случае, ради которого всё заведено: замороженный процесс оттаивает на минуту, тикает и мёрзнет
/// снова. Отметка от тика не двигается — иначе убитому после такого оттаивания процессу следующий
/// запуск насчитал бы одну минуту простоя вместо часа.
/// </para>
/// <para>
...
public readonly record struct BackgroundBeat(DateTimeOffset At, ConnectionState Phase)
{
    ...
    /// <para>
    /// Та же мера служит и обратному: столько же <b>ровного</b> дыхания подряд доказывает, что
    /// процесс работает — раз уж столько подряд пропускает только остановленный.
    /// </para>
    /// </summary>
