...
/// (план 27 §27.2). Каскад сверху вниз: настройка человека → ответ BMS → знание протокола →
/// напряжение вместе с процентом от колеса → догадка по одному напряжению. Ответил кто-то выше —
/// ниже не спускаемся.
/// <para>
...
/// </para>
/// <para>
/// Справочника моделей тут нет и не будет: §27.1а показал, что тесные пары рядов (40/42, 56/60,
/// 30/32) не пересекают границы протоколов, а протокол декодеру известен всегда. Знание о модели
/// приходит готовым числом — ступенью <see cref="CellCountSource.Protocol"/>.
/// </para>
/// <para>
...
public static class CellCountResolver
{
    ...
    /// <summary>
    /// Делитель догадки: «напряжение колеса» в спеках — это заряд под завязку, S × 4,2.
    /// <para>
    /// <b>Тем самым последняя ступень предполагает заряженное колесо</b>, и это осознанное допущение
    /// владельца (08.08.2026), а не побочность деления. Оттого она и последняя: допущение сильное.
    /// </para>
    /// <para>
    /// Врёт оно <b>систематически и всегда в одну сторону — вниз</b>. 24S на 3,5 В дают те же 84 В,
    /// что полный 20S, и деление на 4,2 назовёт 20. Ровно этот случай и открывал план 27.
    /// Безопасным допущение делает не оно само, а вторая половина требования владельца:
    /// <b>число проверяет человек</b>. Оттого кнопка «рассчитать» не пишет молча — она называет
    /// ряд, источник и вольт на банку, а на этой ступени ещё и предупреждает о неполном заряде.
    /// </para>
    /// </summary>
    ...
    public static CellCount Resolve(CellCountInputs inputs)
    {
        ...
        if (inputs.ProtocolCells > 0) return new CellCount(inputs.ProtocolCells.Value, CellCountSource.Protocol);
    ...
    /// границ по вольту на ячейку. <b>Тем самым она предполагает колесо, заряженное почти
    /// полностью</b> (<see cref="FullChargeCellVolts"/>) — допущение владельца, и врёт оно всегда в
    /// меньшую сторону.
    /// <para>
