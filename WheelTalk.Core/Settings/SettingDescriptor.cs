...
public enum SettingsPage
{
    ...
    /// <summary>
    /// Как настроено само колесо — то, что оно рассказало о себе кадром, а не то, что человек задал
    /// нам (план 34 §2). Страница собирается тем же механизмом, что остальные пять, и своего экрана
    /// не получает: её строки — обычные описания с <see cref="SettingDescriptor.ReportedByWheel"/>,
    /// и в наши слои они не пишутся вовсе.
    /// <para>
    /// Отсюда и отличия показа, которые держит <c>SettingsCategoryActivity</c>: переключателя
    /// «Общее / это колесо» у страницы нет — слоёв, между которыми переключать, у неё не бывает.
    /// </para>
    /// </summary>
    WheelDevice,
}
...
public sealed class SettingDescriptor
{
    ...
    /// <summary>
    /// Значение этой настройки в отчёт диагностики не попадает — только факт «задано»
    /// (<see cref="ChangedSettings"/>, план 11 §4.2). Заведено под пароль колеса: отчёт уходит
    /// чужому человеку системным диалогом, и пароль в нём — не отладочная информация.
    /// </summary>
