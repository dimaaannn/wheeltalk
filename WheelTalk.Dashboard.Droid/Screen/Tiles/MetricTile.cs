...
public enum TileKind
{
    ...
    /// <summary>
    /// Разделитель — полоса во всю ширину, отбивающая одну кучку плиток от другой (решение
    /// владельца 11.08.2026). Содержимого у неё нет вовсе, и величина ей не нужна: это не плитка с
    /// пустым числом, а <b>зазор с линией</b>.
    /// <para>
    /// <b>Самостоятельным элементом, а не свойством плитки.</b> Свойство «начинать новую группу»
    /// пробовали первым — владелец его отверг: черта по краю плитки слишком тонка, ничего не
    /// отделяет, да и привязка отступа к соседу неверна по сути. Отступ — это место в раскладке, и
    /// принадлежать он должен раскладке, а не плитке.
    /// </para>
    /// <para>
    /// Занимает всю ширину и <b>свою строку пониже обычной</b> (<c>TilesLayout.DividerRowDp</c>):
    /// ради него укладчик научился строкам разной высоты (<c>TileRows</c>).
    /// </para>
    /// </summary>
    Divider,
    ...
    int? Decimals = null, string Caption = "")
{
    ...
    /// <summary>
    /// Разделитель — всегда во всю ширину и всегда своей высоты: выбирать тут нечего, потому размер
    /// ему не задают ни в меню, ни в раскладке.
    /// </summary>
    public static MetricTile Divider() => new("", TileKind.Divider, new TileSize(TilesLayout.Columns, 1))
    {
        Id = NewId(),
    };
