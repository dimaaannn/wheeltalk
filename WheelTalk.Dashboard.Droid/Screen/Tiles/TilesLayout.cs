...
public static class TilesLayout
{
    ...
    /// <summary>
    /// Высота строки, занятой разделителем. Втрое ниже обычной: это зазор, а не плитка, — но
    /// заметный, ради него разделитель и заведён (решение владельца 11.08.2026, волосяная черта
    /// свойством плитки отвергнута как ничего не отделяющая).
    /// </summary>
    public static int DividerRowDp => 24;

    /// <summary>
    /// Толщина линии разделителя. Тонкая нарочно: отделяет <b>пустота</b>, а линия лишь называет
    /// её — жирная спорила бы с рамками плиток.
    /// </summary>
    public static float DividerLineDp => 1.5f;

    /// <summary>
    /// Густота линии (0…255 от приглушённой краски) — тот же служебный вес, что у ручки правки и
    /// меток шкалы жара: разделитель размечает экран, а не участвует в показаниях.
    /// </summary>
    public static int DividerAlpha => 110;
    ...
    /// <summary>
    /// Отступ метки-подписи от края квадратной плитки — <b>свой, малый</b>, вместо общего поля
    /// (<see cref="PaddingDp"/>): отодвинутая полем подпись съедала у числа заметную часть мелкой
    /// плитки (слова владельца 11.08.2026 — «сдвинуть вверх и влево»).
    /// <para>
    /// Считается от рамки, а не сам по себе: её толщина (<see cref="HeatStrokeDp"/>) плюс зазор —
    /// подпись не вправе коснуться линии ни на прямой стороне, ни в закруглении угла. Полоса жара
    /// живёт на <b>нижней</b> стороне рамки и верхнего угла не трогает, поэтому включена она или
    /// нет — на этот зазор не влияет.
    /// </para>
    /// </summary>
    public static int CornerInsetDp => HeatStrokeDp + 3;

    /// <summary>
    /// Полоска угловой подписи — <b>одно число на разметку и на подбор кегля</b>: его читают и
    /// плитка (отступ числа сверху, <c>TileView.CornerStripPx</c>), и бюджет квадрата
    /// (<c>TileMetrics.SquareLabelPx</c> в <c>TilesScreen</c>). Своей арифметики ни у того, ни у
    /// другого быть не должно: разойдись счёт — знак либо ляжет на число, либо отнимет у него место
    /// впустую.
    /// <para>
    /// Высота — по <b>крупному знаку</b> ▲▼ (<see cref="MarkScale"/>), а не по слову: под него
    /// полоска и строится. Отсчёт идёт от верха содержимого, а подпись сидит выше — на своём малом
    /// отступе, снаружи полей; оттого <see cref="PaddingDp"/> здесь и вычитается. Освободившееся
    /// достаётся числу: подбор кегля увидит больший бюджет тем же числом.
    /// </para>
    /// </summary>
    public static float CornerStripDp =>
    ...
    /// там же пересчитанный замер и довод про широкие экраны.
    /// <para>
    /// <b>Число экрана живёт здесь, а не в ядре.</b> У <c>TileMetrics</c> своё такое же — но оно
    /// умолчание для тех, кто сетку не задаёт (тесты); подбор боевого экрана читает эту ручку.
    /// Поднять одно и забыть другое значит принять решение владельца в расчётах и не показать его
    /// на телефоне.
    /// </para>
    /// </summary>
    public static float SquareRatio => 1.65f;

    /// <summary>
    /// Пол кегля подписи. Ужимая слово под место в углу квадрата, ниже этого не опускаемся: мельче
    /// подпись не читается вовсе, и честнее обрезать её многоточием, чем сделать нечитаемой, — то
    /// же правило, что у пола единицы при числе.
    /// </summary>
    public static int LabelMinSp => 8;
