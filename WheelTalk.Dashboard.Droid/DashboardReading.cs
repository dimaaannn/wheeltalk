...
public sealed record DashboardReading
{
    ...
    /// <summary>
    /// Счётчик поездки: путь от точки отсчёта, которую двигает только рука хозяина — кнопка «Сброс
    /// пиков» в шторке (решение владельца 12.08.2026). <c>null</c> — считать не из чего: колесо
    /// молчит об одометре либо колесо не выбрано вовсе, и центр рисует прочерк, а не ноль. Ноль
    /// читался бы как «никуда не ездили», а это другое утверждение.
    /// <para>
    /// Приходит готовым числом, а не одометром с точкой: точка живёт в хранилище хозяина экрана
    /// (<c>TripPoints</c>) — панель её не читает и не пишет, она рисует.
    /// </para>
    /// </summary>
    public double? TripCounterKm { get; init; }
